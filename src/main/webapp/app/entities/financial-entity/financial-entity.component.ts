...
@Component({
    selector: 'jhi-financial-entity',
    templateUrl: './financial-entity.component.html'
})
export class FinancialEntityComponent implements OnInit, OnDestroy {
    ...
    trackId(index: number, item: FinancialEntity) {
        return item.id;
    }
    registerChangeInFinancialEntities() {
        this.eventSubscriber = this.eventManager.subscribe('financialEntityListModification', (response) => this.reset());
    }
    ...
    private onSuccess(data, headers) {
        ...
        for (let i = 0; i < data.length; i++) {
            this.financialEntities.push(data[i]);
        }
    }
